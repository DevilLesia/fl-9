...
const template = {
	main(todoItems) {
		...
		const addNewBtn = createElement('button', {
			'id': 'add-new-task'
		}, 'Add new task');
		...
		if (items.length) {
			for (let item of items) {
				...
				const checkbox = createElement('button', {
					'class': item.isDone ? 'box-done' : 'box-undone'
				});
				...
				checkbox.onclick = () => {
					if (checkbox.className === 'box-undone') {
						checkbox.className = 'box-done';
...
const route = {
	...
	main() {
		...
		rootNode.innerHTML = '';
		rootNode.appendChild(template.main(items));
	},
